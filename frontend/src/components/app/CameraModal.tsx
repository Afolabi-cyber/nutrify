...
const CameraModal = ({ open, onClose, onCapture }: CameraModalProps) => {
    ...
    try {
      ...
      if (videoRef.current) {
        ...
        try {
          ...
          console.warn("Video playback did not auto-start:", playError);
        }
      }
    } catch (err) {
  ...
  const capturePhoto = () => {
    ...
    if (video.videoWidth === 0 || video.videoHeight === 0) {
      setError("Unable to capture the photo. Please make sure the camera has loaded and try again.");
      return;
    }
    ...
    if (!context) {
      setError("Unable to access drawing context for photo capture.");
      return;
    }
