...
const AppHome = () => {
  ...
  const [notFood, setNotFood] = useState<"not_food" | "not_real_food" | false>(false);
  ...
  const handleProcessImage = async (imageDataUrl: string) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        sessionStorage.setItem("nutrify_food_name", data.food_name || "");
        sessionStorage.setItem("nutrify_macronutrients", JSON.stringify(data.macronutrients || null));
        navigate("/app/ingredients");
      } else {
        if (data.error === "not_food") setNotFood("not_food");
        else if (data.error === "not_real_food") setNotFood("not_real_food");
        ...
        <div className="grid md:grid-cols-2 gap-10 md:gap-16 items-center">
          ...
          >
            ...
            <p className="text-sm md:text-lg text-muted-foreground md:max-w-md mx-auto md:mx-0">
              Snap a photo to instantly discover what's inside, track your nutrition, and get personalized dietary insights.
            </p>
          </motion.div>
          ...
          >
              ...
              <div className="space-y-4">
                <div className="relative w-full aspect-square sm:aspect-video rounded-xl overflow-hidden border border-border/30">
                  ...
                  <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center p-4 backdrop-blur-sm">
                    ...
                    <p className="text-white font-medium text-sm">Identifying ingredients…</p>
                  </div>
                </div>
              </div>
            ...
            /* Not-food state */
              ...
              <div className="space-y-5 text-center py-4">
                <div className="text-5xl">{notFood === "not_real_food" ? "🤖" : "🚫"}</div>
                <div>
                  <p className="font-semibold text-foreground text-base">
                    {notFood === "not_real_food" ? "AI or animated food detected" : "That's not food!"}
                  </p>
                  <p className="text-sm text-muted-foreground mt-1">
                    {notFood === "not_real_food"
                      ? "Please upload a real photo of an actual meal."
                      : "Please upload a photo of a meal or food item."}
                  </p>
                </div>
                <div className="flex flex-col sm:flex-row gap-3 justify-center">
                  {/* Scan — always visible */}
                  <Button
                    className="bg-[#2b5934] hover:bg-[#204427] text-white border-0 rounded-xl h-11 font-medium flex-1"
                    onClick={() => { setNotFood(false); setCameraOpen(true); }}
                  >
                    <Camera className="mr-2 h-4 w-4" /> Scan Again
                  </Button>
                  {/* Upload — desktop only */}
                  <Button
                    className="hidden md:flex bg-[#2b5934] hover:bg-[#204427] text-white border-0 rounded-xl h-11 font-medium flex-1 items-center justify-center"
                    onClick={() => { setNotFood(false); fileInputRef.current?.click(); }}
                  >
            ...
            /* Default idle state */
              ...
              <>
                ...
                {/* Mobile: only Scan label */}
                <h3 className="font-semibold text-[1.1rem] text-foreground mb-1.5 md:hidden">
                  Take a Photo
                </h3>
                {/* Desktop: Scan or Upload label */}
                <h3 className="font-semibold text-[1.1rem] text-foreground mb-1.5 hidden md:block">
                ...
                <div className="flex items-center gap-3 mb-7 opacity-70">
                  ...
                  <span className="text-[11px] uppercase tracking-wider font-medium text-muted-foreground whitespace-nowrap">
                    choose an option
                  </span>
                  <div className="h-px bg-border flex-1" />
                </div>

                {/* Mobile: single full-width Scan button */}
                <div className="md:hidden">
                  <Button
                    className="w-full bg-[#2b5934] hover:bg-[#204427] text-white border-0 rounded-xl h-11 font-medium shadow-sm transition-all"
                    onClick={() => setCameraOpen(true)}
                  >
                    <Camera className="mr-2 h-4 w-4" /> Scan Meal
                  </Button>
                </div>

                {/* Desktop: Scan + Upload grid */}
